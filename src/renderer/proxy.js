...
let url = require('url')
let net = require('net')
let HTTP_PORT = process.argv[2] || 8080 // internal proxy server port
let PROXY_URL = process.argv[3] || null // external proxy server URL
let PROXY_HOST = PROXY_URL ? url.parse(PROXY_URL).hostname : null
let PROXY_PORT = PROXY_URL ? (url.parse(PROXY_URL).port || 80) : null

http.createServer(function onCliReq (cliReq, cliRes) {
  let svrSoc
  let cliSoc = cliReq.socket
  let x = url.parse(cliReq.url)
  let svrReq = http.request({
    host: PROXY_HOST || x.hostname,
    promort: PROXY_PORT || x.port || 80,
  ...
  .on('connect', function onCliConn (cliReq, cliSoc, cliHead) {
    let x = url.parse('https://' + cliReq.url)
    let svrSoc
    if (PROXY_URL) {
      let svrReq = http.request({
        host: PROXY_HOST,
